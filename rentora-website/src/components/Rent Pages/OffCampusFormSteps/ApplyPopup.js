...
const ApplyPopup = ({ user, listing, closePopup, editApplicationData }) => {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const response = await fetch('http://rentora-servers.net:3002/fetch_user_details');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://rentora-servers.net:3200/combine-roommate-applications', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
